...
这些排除规则写在 [.gitignore](.gitignore) 中。它们不会影响本机运行；只是避免把可重新生成的缓存、带隐私风险的临时音频/图片中间文件、大模型和第三方 runtime 推送到公开仓库。

克隆公开仓库后，需要在项目根目录手动放回这些文件，并保持 [config/default.yaml](config/default.yaml) 中的路径一致。Python 虚拟环境通过 `requirements.txt` 复现：
...
本仓库中的代码、脚本和文档使用 [MIT License](LICENSE)。
...
实现文件：[voice_assistant/wake.py](voice_assistant/wake.py)
...
实现文件：[voice_assistant/audio_io.py](voice_assistant/audio_io.py)
...
实现文件：[voice_assistant/asr.py](voice_assistant/asr.py)
...
实现文件：[voice_assistant/intent.py](voice_assistant/intent.py)
...
- [voice_assistant/camera.py](voice_assistant/camera.py)
- [scripts/capture-photo.sh](scripts/capture-photo.sh)
...
实现文件：[voice_assistant/qwen_runner.py](voice_assistant/qwen_runner.py)
...
- [voice_assistant/streaming_tts.py](voice_assistant/streaming_tts.py)
- [voice_assistant/tts.py](voice_assistant/tts.py)
- [voice_assistant/audio_utils.py](voice_assistant/audio_utils.py)
- [voice_assistant/audio_io.py](voice_assistant/audio_io.py)
...
- [voice_assistant/orchestrator.py](voice_assistant/orchestrator.py)
- [voice_assistant/cli.py](voice_assistant/cli.py)
